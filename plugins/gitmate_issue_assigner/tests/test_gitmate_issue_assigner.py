...
class TestIssueLabeller(GitmateTestCase):
    ...
    @patch.object(GitHubIssue, 'assign', new_callable=PropertyMock)
    def test_github(self, m_assign, m_title, m_desc):
        ...
        m_assign.assert_called()
        m_assign.assert_called_with({'bears', 'bear-related'})

    @patch.object(GitLabIssue, 'description', new_callable=PropertyMock)
    @patch.object(GitLabIssue, 'title', new_callable=PropertyMock)
    @patch.object(GitLabIssue, 'assign', new_callable=PropertyMock)
